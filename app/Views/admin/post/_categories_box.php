    ...
    <div class="box-body">
        <?php if (!empty($post)): ?>
            ...
            <div class="form-group m-0">
                ...
                <div id="subcategories_container" class="subcategory-checkboxes">
                    <?php
                    $selectedSubIds = isset($postSubCategoryIds) && is_array($postSubCategoryIds) ? $postSubCategoryIds : (isset($subCategoryId) && !empty($subCategoryId) ? [$subCategoryId] : []);
                    if (!empty($subCategories)):
                        foreach ($subCategories as $item): ?>
                            <div class="checkbox">
                                <label>
                                    <input type="checkbox" name="subcategory_ids[]" value="<?= esc($item->id); ?>" <?= in_array($item->id, $selectedSubIds) ? 'checked' : ''; ?>>
                                    <?= esc($item->name); ?>
                                </label>
                            </div>
                        <?php endforeach;
        ...
        <?php else: ?>
            ...
            <div class="form-group m-0">
                <label class="control-label"><?= trans('subcategory'); ?></label>
                <div id="subcategories_container" class="subcategory-checkboxes"></div>
            </div>
        <?php endif; ?>
    </div>
</div>