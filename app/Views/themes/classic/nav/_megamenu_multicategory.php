...
if (!empty($category)):
   ...
   <li class="nav-item dropdown megamenu-fw mega-li-<?= $category->id; ?> <?= uri_string() == esc($category->name_slug) ? 'active' : ''; ?>">
      <a href="<?= generateCategoryURL($category); ?>" class="nav-link dropdown-toggle" data-toggle="dropdown"><?= esc($category->name); ?> <span class="caret"></span></a>
      <?php if (!empty($categoryPosts) && countItems($categoryPosts) > 0): ?>
         <ul class="dropdown-menu animate fade-up" data-mega-ul="<?= $category->id; ?>">
            <!-- <li> -->
