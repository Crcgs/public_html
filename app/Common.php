...
if (!function_exists('checkUserPermission')) {
    function checkUserPermission($permission)
    {
        if (authCheck()) {
            ...
            $rolePermission = reset($rolePermission); // get first element

            if (!empty($rolePermission) && isset($rolePermission->$permission) && $rolePermission->$permission == 1) {
                return true;
            }
        }
        return false;
...
if (!function_exists('generatePostURL')) {
    function generatePostURL($post, $baseURL = null)
    {
        ...
        if (!empty($post->title_slug)) {
            return site_url($post->title_slug);
        }
