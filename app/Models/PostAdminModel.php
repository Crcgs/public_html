...
class PostAdminModel extends BaseModel
{
    ...
    protected $builderPostCategories;
    ...
    public function __construct()
    {
        ...
        $this->builderPostCategories = $this->db->table('post_categories');
    }
    ...
    public function inputValues()
    {
        return [
            ...
            'latest_news' => inputPost('latest_news'),
            'knowledge_articles' => inputPost('knowledge_articles'),
            'research_paper' => inputPost('research_paper'),
            'opinion_insights' => inputPost('opinion_insights'),
            'glossary' => inputPost('glossary'),
            'notifications_events' => inputPost('notifications_events'),
    ...
    public function setPostData($postType)
    {
        ...
        if (!isset($data['knowledge_articles'])) {
            $data['knowledge_articles'] = 0;
        }

        if (!isset($data['research_paper'])) {
            $data['research_paper'] = 0;
        }

        if (!isset($data['opinion_insights'])) {
            $data['opinion_insights'] = 0;
        }
        ...
        if (!isset($data['notifications_events'])) {
            $data['notifications_events'] = 0;
        }
        ...
        //support multiple subcategories: use the first as main category
        $subCategoryIds = inputPost('subcategory_ids');
        if (is_array($subCategoryIds) && !empty($subCategoryIds)) {
            $firstSub = (int)reset($subCategoryIds);
            if (!empty($firstSub)) {
                $data['category_id'] = $firstSub;
            }
        } else {
    ...
    public function deletePost($id)
    {
        ...
        if (!empty($post)) {
            ...
            $this->deletePostCategories($post->id);
    ...
    //get post category mapping ids
    public function getPostCategoryIds($postId)
    {
        $rows = $this->builderPostCategories->select('category_id')->where('post_id', cleanNumber($postId))->get()->getResult();
        $ids = [];
        if (!empty($rows)) {
            foreach ($rows as $row) {
    ...
    //save post categories from request input (multiple subcategories)
    public function savePostCategoriesFromInput($postId)
    {
        $subCategoryIds = inputPost('subcategory_ids');
        $this->deletePostCategories($postId);
        if (is_array($subCategoryIds) && !empty($subCategoryIds)) {
            $uniqueIds = array_unique(array_map('intval', $subCategoryIds));
            foreach ($uniqueIds as $cid) {
                if (!empty($cid)) {
                    $this->builderPostCategories->insert([
    ...
    //delete mapping rows for a post
    public function deletePostCategories($postId)
    {
        $this->builderPostCategories->where('post_id', cleanNumber($postId))->delete();
    }
