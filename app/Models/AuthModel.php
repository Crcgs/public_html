...
class AuthModel extends BaseModel
{
    ...
    //get active users
    public function getAdminOrAuthorActiveUsers()
    {
        return $this->builder->where('status', 1)->whereIn('role', ['admin', 'author'])->orderBy('username')->get()->getResult();
    }
    ...
    public function editUser($id)
    {
        ...
        if (!empty($user)) {
            $data = [
                ...
                'department' => inputPost('department'),
...
// paginated authors
public function getAuthorsPaginated($perPage, $offset)
{
    return $this->builder
        ->where('role', 'author')
        ->where('status', 1)
