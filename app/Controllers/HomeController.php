...
class HomeController extends BaseController
{
    ...
    public function index()
    {
        ...
        $data['homeTitle'] = $this->settings->home_title;
            ...
            // 🔹 AUTHORS FOR HOME PAGE (TEAM SECTION)
        $authModel = new AuthModel();
        $data['authors'] = $authModel->builder
            ->where('role', 'author')
            ->where('status', 1)
        ...
        $data['latestPosts'] = getLatestPosts(POST_NUM_LOAD_MORE);
    ...
    public function latestNews() {
     ...
     public function researchPaper() {
        ...
        $data['description'] = 'Latest News';
        $data['keywords'] = 'Latest News';
        $data['posts'] = $post = $this->postModel->getPostsByColum('posts.research_paper',1); 
        echo loadView('partials/_header', $data);
        echo loadView('post/research-paper', $data);
        echo loadView('partials/_footer', $data);
    }


     public function opinionInsights() {
        $data = array();
        $data['title'] = 'Latest News';
        $data['description'] = 'Latest News';
        $data['keywords'] = 'Latest News';
        $data['posts'] = $post = $this->postModel->getPostsByColum('posts.opinion_insights',1); 
        echo loadView('partials/_header', $data);
        echo loadView('post/opinion-insights', $data);
        echo loadView('partials/_footer', $data);
    }
...
$data['glossaries'] = $glossaryModel
 ...
 //glossary details
public function glossaryDetail($id)
{
    $glossaryModel = new \App\Models\GlossaryModel();
    ...
    {
        if (empty($post->category_id) || empty($post->user_id)) {
          log_message('error', 'Invalid post data for slug: ' . $post->slug);
          return redirect()->to(langBaseUrl());
        }
    ...
    private function page($page)
    {
        ...
        if ($page->visibility == 0) {
            ...
            } elseif ($page->page_default_name == 'about-us') {
                echo loadView('partials/_header', $data);
                echo loadView('about-us', $data);
                echo loadView('partials/_footer');
            } elseif ($page->page_default_name == 'our-mission') {
                echo loadView('partials/_header', $data);
                echo loadView('our-mission', $data);
                echo loadView('partials/_footer');
            } elseif ($page->page_default_name == 'our-vision') {
                echo loadView('partials/_header', $data);
                echo loadView('our-vision', $data);
                echo loadView('partials/_footer');
            } elseif ($page->page_default_name == 'our-author') {
                 ...
                 $pageNum = (int) ($this->request->getGet('page') ?? 1);
                  ...
                  $totalAuthors = $authModel->getAuthorsCount();
                    $offset = ($pageNum - 1) * $perPage;
                
                    $data['authors'] = $authModel->getAuthorsPaginated($perPage, $offset);
                
                    // pagination info
                    $data['currentPage'] = $pageNum;
                    $data['totalPages']  = ceil($totalAuthors / $perPage);
                    
                echo loadView('partials/_header', $data);
                echo loadView('our-author', $data);
                echo loadView('partials/_footer');
            } else {
                echo loadView('partials/_header', $data);
    ...
    $author = $authModel->builder
     ...
     if (!empty($author)) {
        return redirect()->to(site_url('profile/' . $author->slug));
    }
