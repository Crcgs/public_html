...
function getSubCategories(val) {
    ...
    $.ajax({
        ...
        data: setAjaxData(data),
        success: function (response) {
            // If checkbox container is present (post add/edit), render checkboxes
            if ($('#subcategories_container').length) {
                var $tmp = $('<select></select>').html(response);
                var html = '';
                $tmp.find('option').each(function () {
                    ...
                    if (val && val !== '0') {
                        html += '<div class="checkbox">' +
                            '<label>' +
                            '<input type="checkbox" name="subcategory_ids[]" value="' + val + '"> ' + text +
                            '</label>' +
                            '</div>';
                    }
                });
                $('#subcategories_container').html(html);
            } else if ($('#subcategories').length) {
                // Fallback for other pages that still use dropdown
                $('#subcategories').children('option:not(:first)').remove();
